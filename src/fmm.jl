...
Function calls setup routine of ExaFMM library. Without specific options, Laplace FMM is used.
...
Function assembles the fullrankblocks that contain the closeinteractions of basis and test functions. 
These interactions are faulty when computed by the FMM. 
The faulty values of the FMM are subtracted of the solution using direct evaluations with the same quadrature strategy as used do sample the input points of the FMM.
...
- `nmin`: Minimal number of points in a block of the tree to get split up in eight children blocks (3D).
- `quadstratcbk`: Quadrature strategy used for the correction of the FMM. SafeDoubleNumQStrat() uses a classical Gaussian quadrature and returns zero if test equals trial function.
- `quadstratfbk`: Quadrature strategy for correct evaluation of close interactions. The `BEAST.defaultquadstrat()` can handle equal test and trial functions.
"""
...
Function computes a sparse matrix that convertes the action of test and trial functions onto weighted sums over the quadrature points.
...
Function computes a sparse matrix that convertes the action of test and trial functions onto weighted sums over the quadrature points.
Other than `sample_basisfunctions()` this function computes the curl of the funcitons for each quadrature point.
...
Function computes a sparse matrix that convertes the action of test and trial functions onto weighted sums over the quadrature points.
Other than `sample_basisfunctions()` this function computes divergence of the functions for each quadrature point.
